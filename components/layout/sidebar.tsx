...
import { NAV_ITEMS, SETTINGS_ITEM, activeHref, type NavItem } from "./nav-config";
import { cn } from "@/lib/utils";

interface SidebarProps {
  counts: { pendingPos: number; openDiscrepancies: number; openSoChecks: number };
...
export function Sidebar({ counts, user }: SidebarProps) {
  ...
  const current = activeHref(pathname, [...NAV_ITEMS, SETTINGS_ITEM]);
  return (
    <aside className="hidden w-[248px] shrink-0 flex-col bg-sidebar px-4 py-6 lg:flex">
      ...
      <nav className="mt-8 flex flex-1 flex-col gap-1">
        ...
        <NavLink item={SETTINGS_ITEM} active={SETTINGS_ITEM.href === current} />
      </nav>
